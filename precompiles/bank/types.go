...
import (
	...
	pcommon "github.com/evmos/ethermint/precompiles/common"
)
...
func ParseBalancesArgs(args []interface{}) (sdk.AccAddress, error) {
	if len(args) != 1 {
		return nil, fmt.Errorf(pcommon.ErrInvalidNumberOfArgs, 1, len(args))
	}

	account, ok := args[0].(common.Address)
	if !ok {
		return nil, fmt.Errorf(pcommon.ErrInvalidType, "account", common.Address{}, args[0])
	}
...
func ParseSupplyOfArgs(args []interface{}) (common.Address, error) {
	if len(args) != 1 {
		return common.Address{}, fmt.Errorf(pcommon.ErrInvalidNumberOfArgs, 1, len(args))
	}

	erc20Address, ok := args[0].(common.Address)
	if !ok {
		return common.Address{}, fmt.Errorf(pcommon.ErrInvalidType, "erc20Address", common.Address{}, args[0])
	}

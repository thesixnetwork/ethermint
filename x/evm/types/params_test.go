...
func TestParamsValidate(t *testing.T) {
	testCases := []struct {
		...
		{
			"valid",
			NewParams("ara", true, true, DefaultChainConfig(),DefaultConverParams(),2929, 1884, 1344),
			false,
		},
		{
			"empty",
...
func TestParamsEIPs(t *testing.T) {
	params := NewParams("ara", true, true, DefaultChainConfig(), DefaultConverParams(),2929, 1884, 1344)
	actual := params.EIPs()
